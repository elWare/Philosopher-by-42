...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/22 18:33:42 by jumarque          #+#    #+#             */
/*   Updated: 2025/05/30 08:50:55 by jumarque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_forks(t_data *data)
{
	...
	philos[i].fork_r = &data->forks[data->nb_philos - 1];
	while (++i < data->nb_philos)
	{
		philos[i].fork_l = &data->forks[i];
...
int	ft_init_threads(t_data *data)
{
	...
	if (pthread_create(&data->monit_all_alive, NULL,
		...
		return (FAILURE);
	if (ft_num_meals(data) == true && pthread_create(&data->monit_all_full,
			NULL, &ft_the_watchdog, data))
		return (FAILURE);
	return (SUCCESS);
}
